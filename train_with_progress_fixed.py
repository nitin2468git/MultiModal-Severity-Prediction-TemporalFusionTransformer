#!/usr/bin/env python3
"""
Fixed COVID-19 TFT Training with Progress Display
    ...
    try:
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(train_pbar):
                ...
                batch = trainer._convert_batch(batch)
                ...
                # Update progress bar with safe type conversion
                try:
                    ...
                    # Fallback if learning rate is not a number
                    train_pbar.set_postfix({
                        'Loss': f"{losses['total_loss'].item():.4f}",
                        'LR': "N/A"
                    })
            ...
            # Print epoch results with safe type conversion
            try:
                current_lr = float(trainer.optimizer.param_groups[0]['lr'])
                ...
                print(f"⏱️  Epoch {epoch + 1} completed in {epoch_time:.1f}s")
                print(f"📊 Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f} | LR: N/A")
...
def main():
    ...
    # Step 1: Data Pipeline with larger sample size for better evaluation
    sample_size = 100  # Use 100 patients for better evaluation
